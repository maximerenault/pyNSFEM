...
for i, el in enumerate(F):
    ...
    J = gradN.dot(nodePts).reshape(-1, 2, 2)  # Jacobian of the triangle : GAUSSx2x2
    gradN = np.linalg.solve(J, gradN)  # gradN with respect to (x, y) : GAUSSx2x3
    volumes = 1 / 2 * np.linalg.det(J)  # volumes : GAUSS
    el_A = np.einsum("g, g, gik, gil->kl", gauss_weights, volumes, gradN, gradN)

    N = shape_tri_vec(gauss_points)  # shape function values : GAUSSx3
    el_b = np.einsum("g, g, gj, gk, k->j", gauss_weights, volumes, N, N, nodeSrc)
